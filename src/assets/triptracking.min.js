...
var triptracking={domain_name:"",ac_id:"",trafic_information:{},fb_settings:{apiKey:"AIzaSyBGQzoa71pbK86BcrYhIhAvLsgSuOlPY9M",authDomain:"triptracking-9a432.firebaseapp.com",databaseURL:"https://triptracking-9a432.firebaseio.com",storageBucket:"triptracking-9a432.appspot.com",messagingSenderId:"778722054156"},form_validation_errors:[],setup:function(a,b){var c=this;c.domain_name=window.location.host,c.ac_id=a,firebase.initializeApp(c.fb_settings);var d=new Date,f=(d.getUTCFullYear().toString()+(d.getUTCMonth()<9?"0"+(d.getUTCMonth()+1).toString():(d.getUTCMonth()+1).toString())+(d.getUTCDate()<10?"0"+d.getUTCDate().toString():d.getUTCDate().toString()),this.crumbleCookie().FirstSession);"undefined"==typeof f?this.writeLogic("FirstSession"):this.writeLogic("ReturningSession"),b&&this.addAttributesToForm(b),c.getAllForms()},addAttributesToForm:function(a){for(forms=document.getElementsByClassName("triptracking"),i=0;i<forms.length;i++)forms[i].setAttribute("data-triptracking-id",a)},getAllForms:function(){var a=this;$("form.triptracking").each(function(b){var c=$(this).attr("data-triptracking-id"),d=$(this);$(this).find("input[type=submit]").on("click",function(b){var e=$(this);e.attr("disabled",!0);var f=d.serializeArray();b.preventDefault(),a.onSubmited(c,f).then(function(a){return 1!=a?(e.attr("disabled",!1),!1):(d.submit(),void setTimeout(function(){var a=d.find("input[name=redirect_to]").val();null!=a&&(window.location.href=a)},1e3))})})})},onSubmited:function(a,b){var c=this;return c.getAllPermissibleFields(a).then(function(d){var e=d;return!!c.validateForm(e,b)&&c.createPerson(b,a).then(function(a){return!0})})},validateForm:function(a,b){var d=0,e=[];return a.forEach(function(a){e.push(a.val().name)}),b.forEach(function(a){var b=e.filter(function(b){return b===a.name});b.length<1?a.name.indexOf("_wp")<0&&a.name.indexOf("redirect_to")<0&&(d+=1):a.value.length<1&&(console.log("In the value: "+a.name+" ==> "+a.value),d+=1)}),!(d>0)},createPerson:function(a,b){var c=this,d={};a.forEach(function(a){a.name.indexOf("person")>-1&&(a.name.indexOf("nome")>-1?d[a.name.replace("name","nome")]=a.value:d[a.name]=a.value)});var e=firebase.database().ref().child("/people/");return e.orderByChild("email").equalTo(d.person_email).limitToFirst(1).once("value").then(function(f){if(f.numChildren()>0)f.forEach(function(d){c.createLead(d.key,a,b).then(function(a){return a})});else{var g=JSON.stringify(d);Object.keys(d).forEach(function(a){g=g.replace(a,a.replace("person_",""))}),d=JSON.parse(g);var h=e.push().key;e.child(h).update(d),c.createLead(h,a,b).then(function(a){return a})}})},createLead:function(a,b,c){var d=this,e={};e.data={},b.forEach(function(a){a.name.indexOf("person")<0&&a.name.indexOf("_wp")<0&&a.name.indexOf("redirect_to")<0&&(e.data[a.name]=a.value)}),e.personKey=a,e.formKey=c,e.status=0,d.getTrafficSource("ReturningSession",window.location.href),e.data.analytics={url:window.location.href,page_title:document.title},e.data.analytics.trafic_info=d.trafic_information;var f=firebase.database().ref().child("/leads/"+d.ac_id);return f.push(e).then(function(a){return!0})},getAllPermissibleFields:function(a){return firebase.database().ref().child("/formFields/"+a).once("value")},crumbleCookie:function(a){for(var b=document.cookie.split(";"),c={},d=0;d<b.length;d++){var e=b[d].substring(0,b[d].indexOf("=")).trim(),f=b[d].substring(b[d].indexOf("=")+1,b[d].length).trim();c[e]=f}return a?c[a]?c[a]:null:c},bakeCookie:function(a,b,c,d,e,f){var g=new Date;g.setTime(g.getTime()),c&&(c*=864e5),g=new Date(g.getTime()+c),document.cookie=a+"="+escape(b)+(c?";expires="+g.toGMTString():"")+(d?";path="+d:"")+(e?";domain="+e:"")+(f?";secure":"")},writeLogic:function(a){var b=this.getTrafficSource(a,this.domain_name);b=b.replace(/\|{2,}/g,"|"),b=b.replace(/^\|/,""),b=unescape(b),this.bakeCookie(a,b,182,"/","","")},getParam:function(a,b){try{var c=a.match("[?&]"+b+"=([^&]+)");return c?c[1]:""}catch(a){return""}},calculateTrafficSource:function(){var a="",b="",c="",d="",e="",f=[["bing","q"],["google","q"],["yahoo","q"],["baidu","q"],["yandex","q"],["ask","q"]],g=document.referrer;g=g.substr(g.indexOf("//")+2),ref_domain=g,ref_path="/",ref_search="";var h=document.location.search;if(h.indexOf("utm_source")>-1)a=this.getParam(h,"utm_source"),b=this.getParam(h,"utm_medium"),c=this.getParam(h,"utm_campaign"),d=this.getParam(h,"utm_term"),e=this.getParam(h,"utm_content");else if(this.getParam(h,"gclid"))a="google",b="cpc",c="(not set)";else if(g){g.indexOf("/")>-1&&(ref_domain=g.substr(0,g.indexOf("/")),ref_path=g.substr(g.indexOf("/")),ref_path.indexOf("?")>-1&&(ref_search=ref_path.substr(ref_path.indexOf("?")+1),ref_path=ref_path.substr(0,ref_path.indexOf("?")))),b="referral",a=ref_domain;for(var i=0;i<f.length;i++)if(ref_domain.indexOf(f[i][0])>-1){b="organic",a=f[i][0],d=this.getParam(ref_search,f[i][1])||"(not provided)";break}}return{source:a,medium:b,campaign:c,term:d,content:e}},getTrafficSource:function(a,b){var c=this.calculateTrafficSource(),d=0===c.source.length?"direct":c.source,e=0===c.medium.length?"none":c.medium,f=0===c.campaign.length?"direct":c.campaign;"referral"===e&&(f="");var g=new Date,h="source="+d+"&medium="+e+"&campaign="+f+"&term="+c.term+"&content="+c.content+"&date="+g.toISOString().slice(0,10).replace(/-/g,"");return this.trafic_information={source:d,medium:e,campaign:f,term:c.term,content:c.content},h}};